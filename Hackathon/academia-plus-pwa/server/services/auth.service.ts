...
import type { AuthUserDTO, LoginDTO, LoginResponseDTO, RegisterDTO } from '../dtos/auth.dto.js'
...
export const authService = {
  ...
  register(data: RegisterDTO): AuthServiceResult {
    const email = data.email.trim().toLowerCase()
    const existingUser = usersRepository.findByEmail(email)

    if (existingUser) {
      return { ok: false, status: 409, message: 'E-mail ja cadastrado.' }
    }
